...
import { appNameForAlby, nip5Regexp } from '@/constants/app'
...
export const useNostrStore = create<Nostr>()((set, get) => ({
  ...
  getUser: async (key?: string, relayUrls: string[] = defaultRelays) => {
    ...
    if (nip5Regexp.test(key)) {
      ...
      user = ndk.getUser({ pubkey: profile?.pubkey, relayUrls })
    } else if (key.startsWith('npub')) {
      try {
        const hex = nip19.decode(key)
        if (hex.type !== 'npub') return
        user = ndk.getUser({ pubkey: hex.data, relayUrls })
      } catch (err) {
        return
      }
    } else {
      user = ndk.getUser({ pubkey: key, relayUrls })
    }
    try {
